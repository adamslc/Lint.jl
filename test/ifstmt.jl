...
@test_broken Lint.line(msgs[1]) == 1
...
@test occursin("use of length() in a Boolean context, use isempty()", msgs[1].message)
...
@test_broken occursin("typeof(a) == Int", msgs[1].message)
...
msgs = lintstr(s)
@test_broken msgs[1].code == :I271
@test_broken occursin("typeof(a) == Any", msgs[1].message)
